"""Textos compartidos entre el bot y el servidor de OAuth."""

INSTRUCCIONES_TEXTO = (
    "📖 *Guía de Sebastian*\n\n"
    "Háblele en lenguaje natural (texto o audio 🎤) o mándele una foto 📷. "
    "Esto es todo lo que puede hacer:\n\n"
    "*📋 Tareas*\n"
    "• Agregar hablando: \"anotá comprar pan\", \"tengo que llamar al médico\"\n"
    "• Agregar manual: ponga un punto \".\" adelante del texto. "
    "Ejemplo: \".comprar pan\"\n"
    "• Con fecha: \"agregá pagar la luz, vence el lunes\"\n"
    "• Ver la lista: diga \"mostrame la lista\", o escriba \".tareas\" o \".lista\"\n"
    "• Eliminar: un punto \".\" y el número. Ejemplo: \".2\" (o \"borrá la tarea 2\")\n"
    "• Editar: \"renombrá la tarea 1...\", \"ponele fecha el viernes a la tarea 3\"\n\n"
    "*💸 Gastos*\n"
    "• Registrar: \"gasté 5000 en el súper\", \"pagué 12000 de luz\"\n"
    "• Foto del ticket 📷 → lo registra solo\n"
    "• Ver: \"cuánto gasté este mes\", \"mostrame los gastos de transporte\"\n"
    "• Editar/borrar: \"cambiá el monto del 1 a 4500\", \"borrá el gasto 2\"\n\n"
    "*🔁 Gastos fijos* (se cargan solos cada mes)\n"
    "• \"el alquiler son 200000 por mes\", \"agregá gasto fijo Netflix 5000\"\n"
    "• \"cuáles son mis gastos fijos\", \"cancelá el gasto fijo del club\"\n\n"
    "*💰 Ingresos y balance*\n"
    "• \"cobré 150000\" → registra el ingreso\n"
    "• \"cuál es mi balance\", \"cuánto me queda este mes\"\n\n"
    "*💳 Deudas*\n"
    "• \"le debo 30000 a Vero\", \"Caro me debe 5000\"\n"
    "• \"cuánto debo\", \"mis deudas\", \"saldé la deuda 1\"\n\n"
    "*🛒 Lista del súper*\n"
    "• \"agregá leche y pan a la lista del súper\"\n"
    "• \"pasame la lista del súper\" → la muestra con un botón ➕ para agregar\n"
    "• En modo agregar puede pegar un listado entero (uno por línea o con comas) "
    "y escribir \"listo\" para terminar\n\n"
    "*📝 Listados*\n"
    "• \"armame una lista de viaje con protector, ojotas y toalla\"\n"
    "• \"mostrame el listado del viaje\", \"mis listados\"\n"
    "• Desde el menú puede crear uno e ir dictando los ítems\n\n"
    "*⏰ Recordatorios*\n"
    "• \"recordame llamar al médico mañana a las 3\"\n"
    "• \"avisame a las 18 que saque la carne\"\n"
    "• \"mis recordatorios\" para verlos o cancelarlos\n\n"
    "*📅 Eventos*\n"
    "• Crear: \"reunión el viernes a las 10\" (avisa si se pisa con otro)\n"
    "• Ver: \"qué tengo hoy\", \"qué tengo el martes\"\n"
    "• Editar/eliminar: \"cambiá la hora de la reunión\", \"eliminá el evento X\"\n"
    "• Le aviso ~30 min antes de cada evento ⏰\n\n"
    "*🤔 ¿Tarea, recordatorio o evento?*\n"
    "• Sin hora → 📋 *tarea* (queda en su lista). Ej: \"comprar pan\"\n"
    "• \"recordame/avisame\" + una hora → ⏰ *recordatorio* (le aviso a esa hora). "
    "Ej: \"recordame llamar al médico a las 3\"\n"
    "• Una cita o reunión con día y hora → 📅 *evento* (va al calendario). "
    "Ej: \"agendá reunión el viernes a las 10\"\n\n"
    "*📲 Menú y configuración*\n"
    "• /menu abre todo con botones\n"
    "• En Configuración elige el trato (señor/señora)\n\n"
    "Todos los días a las 8:00 le mando un resumen del día. ☀️"
)
